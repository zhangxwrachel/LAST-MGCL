...
if __name__ == '__main__':
    ...
    opt = torch.optim.Adam(mview.parameters(), lr=lr, weight_decay=weight_decay)
    ...
    for epoch in range(epochs):
        for _ in range(batch_size):
            ...
            opt.zero_grad()
            loss = mview(ori_adj, adj_1, adj_2, ori_features, aug_features1, aug_features2, sparse)
            loss.backward()
            opt.step()
            mview.update_ma()
