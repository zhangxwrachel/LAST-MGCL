...
class MVIEW(nn.Module):
    ...
    def forward(self, adj,feat, aug_feat_1, aug_feat_2):
        ...
        representations_1,online_proj_1 = self.online_encoder(aug_feat_1)
        representations_2,online_proj_2 = self.online_encoder(aug_feat_2)
        ...
        with torch.no_grad():
            representations_11,target_proj_01 = self.target_encoder1(aug_feat_1)
            # print('target_proj_01',target_proj_01)
            representations_12,target_proj_11 = self.target_encoder1(aug_feat_2)
            representations_21,target_proj_02 = self.target_encoder2(aug_feat_1)
            representations_22,target_proj_22 = self.target_encoder2(
                aug_feat_2)
            representations_1,_ = self.target_encoder1(feat)
            representations_2,_ = self.target_encoder2(feat)
        loss_heads_online_1 = multihead_contrastive_loss(representations_0, adj, tau=self.tau)

        loss_heads = loss_heads_online_1
        l_cn_1 = self.alpha * agg_contrastive_loss(online_pred_1, target_proj_01.detach()) +\
            (1.0-self.alpha) * \
            agg_contrastive_loss(online_pred_1, target_proj_02.detach())
        gc.collect()
        torch.cuda.empty_cache()
        l_cn_2 = self.alpha * agg_contrastive_loss(online_pred_2, target_proj_11.detach()) +\
            (1.0-self.alpha) * \
            agg_contrastive_loss(online_pred_2, target_proj_22.detach())
